...
export class Start {
    ...
    constructor(config:Array<string>) {
        ...
        let topWall = new ActorBuilder(actorManager, <Actor>{name:"topWall", color: "#65aaa4", location: {x:-10, y:0.5, z:0}, scale:{x:1, y:1, z:30}});
        let bottomWall = new ActorBuilder(actorManager, <Actor>{name:"bottomWall", color: "#65aaa4", location: {x:10, y:0.5, z:0}, scale:{x:1, y:1, z:30}});
